...
import numpy as np
from numpy import product, cos, sqrt

from Particule import Particule

########## les parametres d'initialisation ###############
dimension = 2  # la dimension du problème
N = 100  # le nombre de particules
Nbiter_max = 400  # le nombre d'iterations
nbiter_cvg = round(Nbiter_max/3)  # le nombre total d'iterations sans convergence
critere_fitness = 1e-3  # critere de fitness
position_min = -600  # position minimale de l'espace de recherche
position_max = 600  # position maximale de l'espace de recherche
cmax = 0.7  # coefficient de contrition
tau = 1.74  # coefficient d'inertie
v_max = 6  # Vitesse maximale
v_min = -6  # Vitesse minimale
spacewidth = 1  # l'espace de recherche
fichier_gif = './pso_griewank.gif'  # le fichier gif de visualisation
...
class PSO:
    ...
    def F(self, p):
        return sum([(x ** 2)/4000 for x in p]) - product([cos(x / sqrt(i)) for x, i in zip(p, range(1, len(p)+1))]) + 1
